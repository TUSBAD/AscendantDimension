#> player:custom_item/undying/use/check

# 捨てられていないか確かめる。捨てられてなかったら非実行フラグを立てる
data modify storage item: check set value false
execute as @e[tag=!TypeChecked,type=item] run function player:custom_item/undying/use/check_item
...
execute if data storage item: {check:false} run function player:custom_item/undying/use/check_inventory
# execute if score _ TUSB matches 2.. run data modify storage item: check set value true

# そしてー、カーソルにあるかどうかも確かめる
execute if data storage item: {check:false} run function player:custom_item/undying/use/check_cursor

# 非実行フラグが折れていたらgive
execute if data storage item: {check:false} run function player:custom_item/undying/use/give
...
# execute as @e[tag=!TypeChecked,type=#lib:projectile] run function player:custom_item/undying/use/check_owner
# 投げられていたならgive function
# execute if data storage _: {_:{use:true}} run function player:custom_item/undying/use/give
