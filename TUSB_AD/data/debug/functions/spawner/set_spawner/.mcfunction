...
# 湧くmobの種類が複数ならmultiを、一種類ならsingleを起動
    execute if data storage _: _.SpawnMob[1] run function debug:spawner/set_spawner/multi
    execute unless data storage _: _.SpawnMob[1] run function debug:spawner/set_spawner/single

# Aura:trueならオーラを乗っける
execute if data storage _: {_:{Aura:true}} as @e[type=armor_stand,tag=this,limit=1] run function debug:spawner/set_spawner/aura

# 情報表示
    function debug:data_get/spawner/name
