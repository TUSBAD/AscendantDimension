...
### 水の有無によって範囲が変わる
execute if score @s TUSB = _ TUSB if entity @s[predicate=!job:is_on_fire] run function skill:act/black_mage/thunder_bolt/schedule/invoke/all
execute if score @s TUSB = _ TUSB unless entity @s[predicate=!job:is_on_fire] run function skill:act/black_mage/thunder_bolt/schedule/invoke/single
