#> skill:act/black_mage/shadow_bolt/attack/

execute store result score _ TUSB run time query gametime
execute as @e[tag=shadowpoint] if score @s TUSB = _ TUSB at @s as @e[distance=..15,type=#entity:mob,tag=Enemy] at @s run function skill:act/black_mage/shadow_bolt/attack/target
