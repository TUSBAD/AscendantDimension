...
execute unless score _ TUSB matches 15.. run tag @s[scores={AreaChangeFlag=11},predicate=entity:player] add NotCaptureIsland
execute unless score _ TUSB matches 15.. run tellraw @s[scores={AreaChangeFlag=11},tag=NotCaptureIsland,predicate=entity:player] {"text": "１５島の攻略が必要です！"}

# アイテム検知
execute if data entity @s Inventory[0] run tag @s[scores={AreaChangeFlag=11},predicate=entity:player] add BringItemError
execute if data entity @s EnderItems[0] run tag @s[scores={AreaChangeFlag=11},predicate=entity:player] add BringItemError
...
### ネザーアスレチックに移動した時
execute if data entity @s Inventory[0] run tag @s[scores={AreaChangeFlag=-90},predicate=player:has_inventory,predicate=entity:player] add BringItemError
