...
# 高難易度からカジュアル以下に下げた場合の警告
    execute if data storage core: difficult.world.change if data storage core: difficult.world{level:another} run tellraw @a ["",{"storage":"core:","nbt":"Prefix.WARN"},{"translate":"%1$sから難易度を変更すると、\n%1$sへ戻せません！","with":[{"nbt":"difficult.another.text","storage":"core:","color":"#FF00FF"}]}]
    execute if data storage core: difficult.world.change if data storage core: difficult.world{level:ascendant} run tellraw @a ["",{"storage":"core:","nbt":"Prefix.WARN"},{"translate":"%1$sから難易度を変更すると、\n%1$sへ戻せません！","with":[{"nbt":"difficult.ascendant.text","storage":"core:","color":"#42c1ff"}]}]
