#> enemy:skill/enemy/game_master/pale_rider/magic
# ペイルライダーのスキル

# me
    me §2§k§r§2ペストクロップ§k§r
# tellraw
    execute as @a[distance=..60] if predicate player:effects/is_poison at @s run tellraw @s {"translate": "§4本当のペストクロップの味はどうだい？"}
# damage
    # 全貫通2048dmg(即死)
    ...
    # attack
        execute as @a[distance=..60] if predicate player:effects/is_poison run function score_damage:api/attack
# sound
